...
class TaobaoChecker(BaseTask):
    ...
    def crawl_store(self, cid, sid):
        ...
        self.fetch(BASE_URL.format(category_id=cid, store_id=sid), args=(sid,), data_type='json', next=self._crawl_category)

    @gen.coroutine
    def crawler_category(self, data, sid):
        BASE_URL = 'http://mobile.walmartmobile.cn:80/MobileHD-0.0.1/items/detail/{upc}'
        items = [i['upc'] for i in data['data']['items']]
        for upc in items:
            self.fetch(BASE_URL.format(upc=upc), data_type='json', next=self._process)
